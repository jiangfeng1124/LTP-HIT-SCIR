...
int DepSRL::LoadResource(const string &ConfigDir) {
	...
	string strPdClassDicName   = ConfigDir + "/verb_class_encode";
	if (! m_srlDictionary.LoadPdClassDic(strPdClassDicName.c_str()) ) return 0;
	
	// load feature set config
	string strFeatSelFileName  = ConfigDir + "/stand_alone_features";
	string strFeatCombFileName = ConfigDir + "/combined_features";
	if (! m_srlConf.LoadFeatSelConf(strFeatSelFileName.c_str())   ) return 0;

	if (! m_srlConf.LoadFeatCombConf(strFeatCombFileName.c_str()) ) return 0;
	
	// construct the FeaturesMethod object
	m_featuresMethod = new FeaturesMethod(&m_srlConf);
	cout << "FeaturesMethod over!" << endl;
	if (! m_featuresMethod ) return 0;
...
int DepSRL::ExtractFeatures(
	...
	DataPreProcess dataPreProc(&ltpData, &m_srlDictionary);

	// extract features !
	SRLBaselineExt srlBaseline(&dataPreProc, m_featuresMethod);
	srlBaseline.SetPredicate(VecAllPredicates);
	srlBaseline.conll_ExtractFeatures(vecAllFeatures, vecAllPos);
