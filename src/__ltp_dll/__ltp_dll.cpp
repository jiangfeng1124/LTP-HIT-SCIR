...
{
	return g_ltp.irlas();
//	return (g_ltp.segmentWord() || g_ltp.postag());
}
...
{
	return g_xml4nlp.GetWSD(WSD_explain, paragraphIdx, sentenceIdx, wordIdx);
}
int	GetWSD(pair<const char *, const char *> &WSD_explain, int globalSentIdx, int wordIdx)
{
	return g_xml4nlp.GetWSD(WSD_explain, globalSentIdx, wordIdx);
}
int	GetWSD(pair<const char *, const char *> &WSD_explain, int globalWordIdx)
{
	return g_xml4nlp.GetWSD(WSD_explain, globalWordIdx);
}
...
// Get WSDs
int _GetWSDsFromSentence(const char **arrWSD, const int sz, int paragraphIdx, int sentenceIdx) {
	vector<const char *> vecWSD(sz);
	if (0 != g_xml4nlp.GetWSDsFromSentence(vecWSD, paragraphIdx, sentenceIdx)) return -1;
	for (int i = 0; i < sz; ++i) arrWSD[i] = vecWSD[i];
	return 0;
}

int _GetWSDsFromSentence(const char **arrWSD, const int sz, int sentenceIdx) {
	vector<const char *> vecWSD(sz);
	if (0 != g_xml4nlp.GetWSDsFromSentence(vecWSD, sentenceIdx)) return -1;
	for (int i = 0; i < sz; ++i) arrWSD[i] = vecWSD[i];
	return 0;
}

int _GetWSDExplainsFromSentence(const char **arrWSDExplain, const int sz, int paragraphIdx, int sentenceIdx)
{
	vector<const char *> vecWSDEx(sz);
	if (0 != g_xml4nlp.GetWSDExplainsFromSentence(vecWSDEx, paragraphIdx, sentenceIdx)) return -1;
	for (int i = 0; i < sz; ++i) arrWSDExplain[i] = vecWSDEx[i];
	return 0;
}

int _GetWSDExplainsFromSentence(const char **arrWSDExplain, const int sz, int sentenceIdx)
{
	vector<const char *> vecWSDEx(sz);
	if (0 != g_xml4nlp.GetWSDExplainsFromSentence(vecWSDEx, sentenceIdx)) return -1;
	for (int i = 0; i < sz; ++i) arrWSDExplain[i] = vecWSDEx[i];
	return 0;
}
